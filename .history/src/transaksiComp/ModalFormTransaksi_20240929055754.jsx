...
const ModalFormTransaksi = ({ onClose }) => {
  ...
  const [error, setError] = useState(null);
   ...
   ewa (selisih) saat tanggal berubah
  useEffect(() => {
   ...
   il saat MobilId berubah
  useEffect(() => {
    const fetchMobilPrice = async () => {
      if (formData.MobilId) {
        try {
          const response = await getMobilById(formData.MobilId);  bil berdasarkan ID
          setMobilPrice(response.harga);  n memiliki field 'harga'
        } catch (err) {
          console.error('Error mengambil harga mobil:', err);
          setMobilPrice(0);
        }
      } else {
        setMobilPrice(0);  a MobilId yang dipilih
      }
    };
   ...
   tal saat selisih atau harga mobil berubah
  useEffect(() => {
    setFormData((prevData) => ({
      ...prevData,
