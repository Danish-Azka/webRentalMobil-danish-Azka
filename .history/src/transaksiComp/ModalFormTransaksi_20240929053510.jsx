...
import { getMobil } from '../service/apiMobil'; // Mengasumsikan kamu memiliki API untuk mengambil data mobil
...
const ModalFormTransaksi = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchMobilPrice = async () => {
      if (formData.MobilId) {
        try {
          const response = await getMobil(formData.MobilId); // Mengambil detail mobil berdasarkan ID
  ...
  useEffect(() => {
    setFormData((prevData) => ({
      ...prevData,
      totalBiaya: selisih * formData.MobilId.Mobil.harga
    }));
  }, [selisih, formData.MobilId.Mobil.harga]);
